...
def load_mimic(base_dir: str = 'experiments/data'):
...
def load_cub2(base_dir='experiments/data'):
    ...
    y = train_data.iloc[:, -1].values
    # concept_names = [f'feature{i:03}' for i in range(x.shape[1])]
    concept_names = np.load(os.path.join(base_dir, 'CUB/concept_names.npy'))
...
def add_noise(src_dataset, noise_type, noise_ratio, noise_degree):
    if src_dataset == 'cub':
    ...
    # Calculate the split sizes based on the ratio
    total_size = len(x)
    split_sizes = [int(noise_ratio * total_size), total_size - int(noise_ratio * total_size)]

    # Randomly split the tensors while maintaining the same split index
    splits = random_split(range(total_size), split_sizes)

    # Get the indices of the splits
    split_indices = [split.indices for split in splits]

    # Access the noise based on noise ratio
    x_noise = x[split_indices[0]]
    y_noise = y[split_indices[0]]

    if noise_type == 'feature':
        # all input features value in the range of (0, 1)
        noise_mean = 0.0
        noise_std = noise_degree  # Adjust the standard deviation as needed

        # Generate Gaussian noise of the same shape as 'x'
        noise = torch.FloatTensor(np.random.normal(loc=noise_mean, scale=noise_std, size=x_noise.shape))
        x_noise = x_noise + noise
    else:
        # feature type is label, shuffle the label to generate random labels
        y_noise_pure = one_hot(torch.randint(0, y_noise.shape[1], (y_noise.shape[0],)).to(torch.long)).to(torch.float)
        # Set the probability of choosing noise or original y data with the possibility of noise_degree
        # Create a random mask of the same size as the tensors
        mask = torch.rand(y_noise_pure.size(0)) < noise_degree

        # Create the new tensor by selecting rows based on the mask
        y_noise = torch.where(mask[:, None], y_noise_pure, y_noise)
